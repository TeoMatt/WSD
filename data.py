...
rm_satopen = re.compile("""<sat.*?>""")
rm_satclose = re.compile("""</sat>""")
...
def clean_context(ctx_in):
    ...
    ctx = rm_misc.sub('', ctx)
    ctx = rm_satopen.sub('', ctx)
    ctx = rm_satclose.sub('', ctx)
    return ctx
...
def build_context(data, word_to_id):
    ...
    #print(target_sense_to_context)
    #print(len(target_sense_to_context))
...
def get_embedding(sense_embedding_file):
    ...
    with open(sense_embedding_file, 'r') as f:
        reader = csv.reader(f)
    ...
    for key, value in sense_embeddings_.items():
        ...
        sense_embeddings__[key] = new_vec    
    return sense_embeddings__

def get_embeddingBIS(sense_embedding_file):
    sense_embeddings_ = None
    with open(sense_embedding_file, 'r') as f:
...
def get_keys(key_file):
    ...
    for key, value in sense_embeddings_.items():
        key = key.split(" ")[1]        
        sense_embeddings__[key] = value    
    return sense_embeddings__

def get_embeddingB(sense_embedding_file):
    sense_embeddings_ = None
    with open(sense_embedding_file, 'r') as f:
        reader = csv.reader(f)
        sense_embeddings_ = dict(reader)
...
def convert_to_numeric(data, word_to_id, target_word_to_id, target_sense_to_id, n_senses_from_word_id, target_sense_to_context_embedding, is_training=True):
    ...
    for instance in data:
     ...
     #   print(len(_instance))
            ...
            _instance.append(target_id)
            target_sense = instance['target_sense']
            senses = target_sense_to_id[target_id]
            sense_id = senses[target_sense] if target_sense else -1
            _instance.append(sense_id)
            _instance.append(one_hot_encode(n_senses_from_word_id[target_id], sense_id))
...
def convert_to_numericAlpha(data, word_to_id, target_word_to_id, target_sense_to_id, n_senses_from_target_id):
    ...
    for i in range(n_batches):
        ...
        for j in range(batch_size):
            if i * batch_size + j < data_len:
                n_to_use_f = min(n_step_f, len(batch[j][0]))
                n_to_use_b = min(n_step_b, len(batch[j][1]))
                if n_to_use_f:
                    xfs[j, -n_to_use_f:] = batch[j][0][-n_to_use_f:]
                if n_to_use_b:
                    xbs[j, -n_to_use_b:] = batch[j][1][-n_to_use_b:]
        ...
        instance_ids = [inst[2] for inst in batch]
        # labels
        target_ids = [inst[3] for inst in batch]
        sense_ids = [inst[4] for inst in batch]
